...
func (g *Game) pipColor(cid cardid.CardID) color.RGBA {
	...
	if g.settings.ColorfulCards {
		...
		case 4:
			...
			case cardid.CLUB:
				return ExtendedColors[g.settings.ClubColor]
			case cardid.DIAMOND:
				return ExtendedColors[g.settings.DiamondColor]
			case cardid.HEART:
				return ExtendedColors[g.settings.HeartColor]
			case cardid.SPADE:
				return ExtendedColors[g.settings.SpadeColor]
			}
		case 2:
			switch suit {
			case cardid.NOSUIT:
			...
			case cardid.CLUB, cardid.SPADE:
				return ExtendedColors[g.settings.BlackColor]
			case cardid.DIAMOND, cardid.HEART:
				return ExtendedColors[g.settings.RedColor]
			}
		case 1:
			return ExtendedColors[g.settings.SpadeColor]
		}
...
func (g *Game) createCardImages() {
	...
	MovableCardBackImage = createCardBackImage(g.settings.MovableCardBackColor)
